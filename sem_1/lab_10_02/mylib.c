    ...
    for (int i = *amount - 1; i >= 0; i--) {
        ...
        if (counter > 2) {
            if (*amount == 1) {
                ...
                *amount = *amount - 1;
                class = (Student*)realloc(class, *amount * sizeof(Student));
            }
        }
    }
    return class;
...
void findAvgMark(Student* class, const int amount) {
    for (int i = 0; i < amount; i++) {
        float averageMark = class[i].marks[0];
        for (int j = 1; j < AMOUNT_OF_MARKS; j++) {
            averageMark += class[i].marks[j];
        }
        class[i].averageMark = roundf(averageMark * 10 / 4) / 10;
    }
}
...
void sortByAverageMark(Student* class, int amount) {
    for (int i = 0; i < amount - 1; i++) {
        for (int j = i + 1; j < amount; j++) {
            if (class[i].averageMark < class[j].averageMark) {
